export type FormFieldType = "select" | "checkbox" | "text" | "url" | "textarea";
...
export interface FormField {
  ...
  label: string;
  /** Dot-path key into the platform's form state, e.g. `tk_safety.allow_comments`. */
  name: string;
...
export const platformFormConfigs: Record<string, PlatformConfig> = {
  tiktok: {
    fields: [
      ...
      { type: "checkbox", label: "Allow duets", name: "tk_safety.allow_duet" },
      {
        type: "checkbox",
      ...
      {
        type: "checkbox",
        label: "Your brand content",
        name: "tk_promotion.is_your_brand_content",
        defaultValue: false,
        conditional: { field: "tk_promotion.self_promotion", value: true },
      },
      {
        type: "checkbox",
        label: "Branded content (from another brand)",
  ...
  facebook: {
    fields: [
      ...
      { type: "url", label: "Link (optional)", name: "fb_link" },
      {
        type: "text",
        label: "Place ID (optional)",
        name: "fb_options.reel.place_id",
        conditional: { field: "fb_type", value: "reel" },
      },
    ],
    sections: [
      { fields: ["fb_type"] },
  ...
  youtube: {
    fields: [
      ...
      { type: "text", label: "Tags (optional)", name: "yt_tags" },
      { type: "text", label: "Category ID (optional)", name: "yt_category_id" },
      {
        type: "checkbox",
        label: "Made for kids",
...
/**
 * Platforms with no per-platform knobs still appear on the composer,
 * but render without a settings card.
 */
