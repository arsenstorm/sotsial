...
function CopyButton({ code }: { readonly code: string }) {
  ...
  return (
    <button
      className={clsx(
        "group/button absolute top-3.5 right-4 overflow-hidden rounded-full py-1 pr-3 pl-2 font-medium text-2xs opacity-0 backdrop-blur transition focus:opacity-100 group-hover:opacity-100",
        copied
...
function CodePanel({
  ...
  return (
    <div className="group dark:bg-white/2.5">
      <CodePanelHeader label={label} tag={tag} />
      <div className="relative">
        <pre className="overflow-x-auto p-4 text-white text-xs">{children}</pre>
...
function CodeGroupHeader({
  ...
  return (
    <div className="flex min-h-[calc(theme(spacing.12)+1px)] flex-wrap items-start gap-x-4 border-zinc-700 border-b bg-zinc-800 px-4 dark:border-zinc-800 dark:bg-transparent">
      {title && (
      ...
      {hasTabs && (
        <TabList className="-mb-px flex gap-4 font-medium text-xs">
          {Children.map(children, (child, childIndex) => (
            <Tab
              className={clsx(
                "border-b py-3 ui-not-focus-visible:outline-none transition",
...
export function CodeGroup({
  ...
  const header = (
    <CodeGroupHeader selectedIndex={tabGroupProps.selectedIndex} title={title}>
      {children}
    </CodeGroupHeader>
  );
